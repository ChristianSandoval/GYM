...
import com.openbravo.pos.inventory.StockRemanenteInfo;
...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    public DataLogicSales() {
        stockdiaryDatas = new Datas[] {Datas.STRING, Datas.TIMESTAMP, Datas.INT, Datas.STRING, Datas.STRING, Datas.STRING, Datas.DOUBLE, Datas.DOUBLE,Datas.STRING};
        ...
        productsRow = new Row(
                ...
                new Field("STOCKVOLUME", Datas.STRING, Formats.STRING),
                ...
                new Field("DIAS", Datas.INT, Formats.INT));
    }
    ...
    public final boolean getCustomer(String id)
    throws BasicException
    {
      return new PreparedSentence(this.s, "SELECT ID FROM CUSTOMERS WHERE ID = ?", SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE).find(id) != null;
    }

public CustomerInfoExt findCustomerExtById(String id)
    throws BasicException
    {
      return (CustomerInfoExt)new PreparedSentence(this.s, "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT, FIRSTNAME, LASTNAME, EMAIL, PHONE, PHONE2, FAX, ADDRESS, ADDRESS2, POSTAL, CITY, REGION, COUNTRY FROM CUSTOMERS WHERE ID = ? AND VISIBLE = " + this.s.DB.TRUE(), SerializerWriteString.INSTANCE, new CustomerExtRead()).find(id);
    }
    ...
    public final ProductInfoExt getProductInfo(String id) throws BasicException {
        return (ProductInfoExt) new PreparedSentence(s
            , "SELECT ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, TAXCAT, CATEGORY, ATTRIBUTESET_ID, IMAGE, ATTRIBUTES, STOCKVOLUME, DAYS " +
    ...
    public List<ProductInfoExt> getProductCatalog(String category) throws BasicException  {
        return new PreparedSentence(s
            , "SELECT P.ID, P.REFERENCE, P.CODE, P.NAME, P.ISCOM, P.ISSCALE, P.PRICEBUY, P.PRICESELL, P.TAXCAT, P.CATEGORY, P.ATTRIBUTESET_ID, P.IMAGE, P.ATTRIBUTES, P.STOCKVOLUME, P.DAYS " +
              "FROM PRODUCTS P, PRODUCTS_CAT O WHERE P.ID = O.PRODUCT AND P.CATEGORY = ? " +
              "ORDER BY O.CATORDER, P.NAME"
                    //"ORDER BY P.PRICESELL"
            , SerializerWriteString.INSTANCE
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                SentenceExec ticketlineinsert = new PreparedSentence(s
                    , "INSERT INTO TICKETLINES (TICKET, LINE, PRODUCT, ATTRIBUTESETINSTANCE_ID, UNITS, PRICE, TAXID, ATTRIBUTES,TOTAL) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)"
                ...
                for (TicketLineInfo l : ticket.getLines()) {
                    ...
                    if (l.getProductID() != null)  {
                        ...
                        getStockDiaryInsert().exec(new Object[] {
                            ...
                            new Double(l.getPrice()), String.valueOf(ticket.getProperty("sequence") )
                        });
                        
                        ProductInfoExt productInfo = DataLogicSales.this.getProductInfo(l.getProductID());
                        if(productInfo.getCategoryID().equals("0")&&ticket.getTicketType()==TicketInfo.RECEIPT_NORMAL)
                        {
                            getDateUpdate().exec(new DataParams() { public void writeValues() throws BasicException {
                                setInt(1, productInfo.getDays());
                                setString(2, ticket.getCustomer().getId());
                            }});
                        }
                        
                        Properties properties = productInfo.getProperties();
                        ...
                        while (count < properties.size())
                        {
                          ...
                          ProductInfoExt prodTemp = DataLogicSales.this.getProductInfo(key);
                          getStockDiaryInsert().exec(new Object[] { UUID.randomUUID().toString(), ticket.getDate(), MovementReason.OUT_SALE.getKey(), location, key, null, new Double(-Double.parseDouble(element) * l.getMultiply()), new Double(productInfo.getPriceBuy()),String.valueOf(ticket.getProperty("sequence")) });
    ...
    public int setAbreCajon(final TicketInfo ticket) throws BasicException
    {
        return new PreparedSentence(s
                    , "INSERT INTO CAJON (FECHA,USUARIO) VALUES (?, ?)"
                    , SerializerWriteParams.INSTANCE
                    ).exec(new DataParams() { public void writeValues() throws BasicException {
                        setTimestamp(1, new Date());
                        setString(2, ticket.getUser().getName());
                    }});
    }
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                if (updateresult == 0) {
                 ...
                 new PreparedSentence(s
                    , "INSERT INTO STOCKDIARY (ID, DATENEW, REASON, LOCATION, PRODUCT, ATTRIBUTESETINSTANCE_ID, UNITS, PRICE, ACTIVECASH) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
*/
    ...
    public final SentenceList getProductCatQBF() {
        return new StaticSentence(s
            , new QBFBuilder(
                "SELECT P.ID, P.REFERENCE, P.CODE, P.NAME, P.ISCOM, P.ISSCALE, P.PRICEBUY, P.PRICESELL, P.CATEGORY, P.TAXCAT, P.ATTRIBUTESET_ID, P.IMAGE, P.STOCKCOST, P.STOCKVOLUME, CASE WHEN C.PRODUCT IS NULL THEN " + s.DB.FALSE() + " ELSE " + s.DB.TRUE() + " END, C.CATORDER, P.ATTRIBUTES, P.DAYS " +
        ...
        return new SentenceExecTransaction(s) {
            public int execInTransaction(Object params) throws BasicException {
                ...
                int i = new PreparedSentence(s
                    , "INSERT INTO PRODUCTS (ID, REFERENCE, CODE, NAME, ISCOM, ISSCALE, PRICEBUY, PRICESELL, CATEGORY, TAXCAT, ATTRIBUTESET_ID, IMAGE, STOCKCOST, STOCKVOLUME, ATTRIBUTES, DAYS) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ...
    public final SentenceExec getProductCatUpdate() {
        return new SentenceExecTransaction(s) {
            public int execInTransaction(Object params) throws BasicException {
                Object[] values = (Object[]) params;
                int i = new PreparedSentence(s
                    , "UPDATE PRODUCTS SET ID = ?, REFERENCE = ?, CODE = ?, NAME = ?, ISCOM = ?, ISSCALE = ?, PRICEBUY = ?, PRICESELL = ?, CATEGORY = ?, TAXCAT = ?, ATTRIBUTESET_ID = ?, IMAGE = ?, STOCKCOST = ?, STOCKVOLUME = ?, ATTRIBUTES = ?, DIAS = ? WHERE ID = ?"
    ...
    public final SentenceExec getDebtUpdate() {
        ...
        return new PreparedSentence(s
                , "UPDATE CUSTOMERS SET CURDEBT = ? WHERE ID = ?"
                , SerializerWriteParams.INSTANCE);
    }
    
    public final SentenceExec getDateUpdate() {

        return new PreparedSentence(s
                , "UPDATE CUSTOMERS SET CURDATE = CURDATE+INTERVAL ? DAY WHERE ID = ?"
                , SerializerWriteParams.INSTANCE);
    }

    public final SentenceExec setRegistro() {
        return new PreparedSentence(s
                , "INSERT INTO REGISTRO(CUSTOMER, DATEREG, DATEEND) VALUES (?,NOW(),?)"
                , SerializerWriteParams.INSTANCE);
    }
    ...
    public final double findProductStockSimple(String id) throws BasicException {

        PreparedSentence p = new PreparedSentence(s, "SELECT UNITS FROM STOCKCURRENT WHERE PRODUCT = ?"
                    , new SerializerWriteBasic(Datas.STRING)
                    , SerializerReadDouble.INSTANCE);

        Double d = (Double) p.find(id);
        return d == null ? 0.0 : d.doubleValue();
    }
    ...
    protected static class CustomerExtRead implements SerializerRead {
        public Object readValues(DataRead dr) throws BasicException {
            ...
            c.setVigencia(dr.getTimestamp(10));
